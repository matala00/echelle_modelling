...
def tracing(m, l0):
    ...
    T_slit = np.asarray([-8.5*np.pi/180,0.0,-8.5*np.pi/180])
    ...
    #Orientation
    T_echelle = np.asarray([-1.2*np.pi/180, -75.2*np.pi/180,0.0])
    ...
    SC_grism = refraction_index.SC('Infrasil')   #We load grims material Sellmeier coefficients
    dGG = 0.0 #-0.018
    GD = np.asarray([1,  (80.0 + dGG)*1e-3])
    ...
    #Position
    d_gr_cam = -49.992
    H8 = trace.to_next_surface3(H7, DC7, d_gr_cam)
    x.append(H8[2])
    y.append(H8[1])
    z.append(H8[0])
    ...
    '''
    #return m, l0, H1[0], H1[1], H1[2], DC1[0], DC1[1], DC1[2]
