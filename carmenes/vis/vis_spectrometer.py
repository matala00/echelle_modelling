...
def tracing(m, l0):
    ...
    #orientation
    apex = 17.8*np.pi/180 #deg 17.8
    T_grism_in = np.asarray([8.4*np.pi/180, 0.0*np.pi/180, 0.0*np.pi/180])
    T_grism_out = np.asarray([T_grism_in[0] + apex, T_grism_in[1], T_grism_in[2]])
    ...
    T_cam = np.asarray([(1.835 + dalpha_cam)*np.pi/180, (0.0+dbeta_cam)*np.pi/180, (0.0+dgamma_cam)*np.pi/180])
